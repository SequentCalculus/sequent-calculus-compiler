...
def map_list(f : FunExprExpr , l : ListExpr) : ListExpr := l.case { 
  Nil => Nil,
  Cons(x : Expr, xs : ListExpr) => Cons(f.ApE(x), map_list(f, xs)) };

def map_expr(f:FunExprExpr,e:Expr) : Expr := e.case{
...
def app_list(f:FunExprUnit,l:ListExpr) : Unit := l.case {
  Nil => Unit,
  Cons(x:Expr,xs:ListExpr) => let x : Unit = f.ApU(x) in app_list(f,xs)
};

def app_exp(f:FunExprUnit,e:Expr) : Unit := e.case{
...
def equal_list(l1:ListExpr,l2:ListExpr) : Bool := l1.case{
  Nil => l1.case {
    Nil => True,
...
def equal(exp1:Expr,exp2:Expr) : Bool := exp1.case {
  ...
  Num(i1:i64) => exp2.case{
    Num(i2:i64) => ife(i1,i2,True,False),
    Add(sums:ListExpr) => False,
    Sub(subs:ListExpr) => False,
    Mul(muls:ListExpr) => False,
    Div(divs:ListExpr) => False,
...
def deriv() : FunExprExpr := cocase { ApE(e:Expr) => 
  e.case{
