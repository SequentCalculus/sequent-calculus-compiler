...
def p(n: i64, perms: List[List[i64]], x: List[i64]): Pair[List[List[i64]], List[i64]] {
    if 1 < n { loop_p(n - 1, perms, x, n) } else { Tup(perms, x) }
}
...
def rev_loop(x: List[i64], n: i64, y: List[i64]): List[i64] {
    if n == 0 { y } else { rev_loop(tail_i(x), n - 1, Cons(head_i(x), y)) }
}

def list_tail(x: List[i64], n: i64): List[i64] {
    if n == 0 { x } else { list_tail(tail_i(x), n - 1) }
}
...
def loop_one2n(n: i64, p: List[i64]): List[i64] {
    if n == 0 { p } else { loop_one2n(n - 1, Cons(n, p)) }
}
...
def loop_work(m: i64, perms: List[List[i64]]): List[List[i64]] {
    if m == 0 { perms } else { loop_work(m - 1, permutations(head_l(perms))) }
}
...
def perm9(m: i64, n: i64): Bool {
    run_benchmark(
        ...
        new {
            apply(result) =>
                if sumlists(result) == (((n * (n + 1)) * factorial(n)) / 2) { True } else { False }
        }
    )
}
