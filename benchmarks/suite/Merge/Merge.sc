...
def tabulate(n:i64, f:FunI64I64) : ListI64 { 
  if n==0 {
    ...
    Cons(f.Ap(n),tabulate(n-1,f))
  }
}

def rev(lold:ListI64,lnew:ListI64) : ListI64 {
  lold.case{
    Nil => lnew,
...
def head(l:ListI64) : i64 {
  l.case { Nil => -1, Cons(x:i64,xs:ListI64) => x }
}

def main(n:i64) : i64 {
  let l1 : ListI64 = rev(tabulate(n,cocase{Ap(x:i64) => 2*x}),Nil);
  let l2 : ListI64 = rev(tabulate(n,cocase{Ap(x:i64) => (2*x)+1}),Nil);
  head(merge(l1,l2))
}
