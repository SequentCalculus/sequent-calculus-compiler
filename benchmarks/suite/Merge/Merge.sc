...
def tabulate_loop(n: i64, len: i64, f: Fun[i64, i64], acc: List[i64]): List[i64] {
    if n < len { tabulate_loop(n + 1, len, f, Cons(f.apply[i64, i64](n), acc)) } else { rev(acc) }
}

def tabulate(n: i64, f: Fun[i64, i64]): List[i64] {
    if n < 0 { Nil } else { tabulate_loop(0, n, f, Nil) }
}
    ...
    l1.case[i64] {
        ...
        Cons(x1, xs1) =>
            l2.case[i64] {
                Nil => l1,
                Cons(x2, xs2) =>
                    if x1 <= x2 { Cons(x1, merge(xs1, l2)) } else { Cons(x2, merge(l1, xs2)) }
            }
    }
}
