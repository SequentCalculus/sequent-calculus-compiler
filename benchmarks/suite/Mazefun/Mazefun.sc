...
def tup1(tup:PairII) : i64 := tup.case { Tup(fst:i64,snd:i64) => fst };

def tup2(tup:PairII) : i64 := tup.case { Tup(fst:i64,snd:i64) => snd };

def fst(pt:Pt) : i64 := pt.case {
  Empty => 0, //should probably give a runtime error
  Pt(x:i64,y:i64) => x
};

def snd(pt:Pt) : i64 := pt.case {
  Empty => 0, //should probably give a runtime error
  Pt(x:i64,y:i64) => y
};

def pt_eq(pt1:Pt,pt2:Pt) : Bool := pt1.case{
  Empty => pt2.case{
    Empty => True,
  ...
  Pt(x1:i64,y1:i64) => pt2.case{
    Empty => False,
    Pt(x2:i64,y2:i64) => ife(x1,x2,ife(y1,y2,True,False),False)
  }
};

def fori(start:i64,end:i64,fun:FunIntPt) : ListPt := ife(start,end,Nil,
...
def append(l1:ListPt,l2:ListPt) : ListPt := l1.case{
  Nil => l2,
  Cons(pt:Pt,pts:ListPt) => Cons(pt,append(pts,l2))
};

def concat(l:ListListPt) : ListPt := l.case{
  NilL => Nil,
  ConsL(lpt:ListPt,lpts:ListListPts) => append(filter_empty(lpt),concat(lpts))
};

def list_read(lst:ListPt,i:i64) : Pt :=  lst.case {
  Nil => Empty,
  Cons(pt:Pt,pts:ListPt) => ifz(i,pt,list_read(pts,(i-1)))
};

def list_write(lst:ListPt,i:i64, new_pt:Pt) : ListPt := lst.case{
  Nil=>Nil,
  Cons(pt:Pt,pts:ListPt) => ifz(i,Cons(new_pt,pts),Cons(pt,list_write(pts,i-1,new_pt)))
};

def list_remove_pos(lst:ListPt,i:i64) : ListPt := lst.case {
  Nil => Nil, // should probably be a runtime error 
  Cons(pt:Pt,pts:Pts) => ifz(i, pts, Cons(pt,list_remove_pos(pts,(i-1))))
};

def is_empty(lst:ListPt) : Bool := lst.case{
  Nil => True,
...
def member(lst:ListPt,pt:Pt) : Bool := lst.case{
  Nil => false,
  Cons(pt1:Pt,pts:ListPt) => or(pt_eq(pt1,pt),member(pts,pt))
};

def has_duplicates(lst:ListPt) : Bool := lst.case {
  Nil => False,
  Cons(pt:Pt, pts:ListPt) => or(member(pts,pt),has_duplicates(xs))
};

def len_l(lst:ListListPt) : i64 := lst.case {
  Nil => 0,
  Cons(lpt:ListPt,lpts:ListListPt) => 1+len_l(lpts)
};

def next_random(cur:i64) : i64 :=  ((cur * 3581) + 12751) % 131072;

// needs to be toplevel because of no term-level recursion
def shuf(lst:ListPt,rand:i64) : ListPt := is_empty(lst).case{
  True => Nil,
  False =>  let new_rand : i64 = next_random(rand) in 
      let i : i64 = new_rand % len(lst) in
      Cons(list_read(lst,i),shuf(list_remove_pos(lst,i),new_rand))
};
                         
def shuffle(lst:ListPt) : ListPt := shuf(lst,0);

def neighboring_cavities(pos:Pt,cave:ListListPt) : ListPt := 
  let size : PairII = matrix_size(cave) in
  let n : i64 = tup1(size) in 
  let m : i64 = tup2(size) in 
  let i : i64 = fst(pos) in 
  let j : i64 = snd(pos) in
  ...
  concat( 
  ConsL(ifl(1,i,   not_empty.ApIIB(i,  j  ).case {True => Cons(Pt(i-1, j),  Nil), False => Nil}, Nil),
  ConsL(ifl(i,n-1, not_empty.ApIIB(i+1,j  ).case {True => Cons(Pt(i+1, j),  Nil), False => Nil}, Nil),
  ConsL(ifl(1,j,   not_empty.ApIIB(i,  j-1).case {True => Cons(Pt(i,   j-1),Nil), False => Nil}, Nil),
  ConsL(ifl(j,m-1, not_empty.ApIIB(i,  j+1).case {True => Cons(Pt(i,   j+1),Nil), False => Nil}, Nil),
    NilL)))));
...
def change(cave:ListPtPt,pos:Pt,new_id:Pt,old_id:Pt) : ListPtPt := 
  let i : i64 = fst(pos) in 
  let j : i64 = snd(pos) in 
  let cavityID : Pt = matrix_read(cave,i,j) in   
...
def pierce(pos:Pt,cave : ListPtPt) : ListListPt := matrix_write(cave,fst(pos),snd(pos),pos);

def try_to_pierce(pos:Pt,cave:ListListPt) : ListListPt := 
  let ncs : ListPt = neighboring_cavities(pos,cave) in       
...
def pierce_randomly(possible_holes:ListPt,cave:ListListPt) : ListLisPt := possibleHoles.case{
  Nil => cave,
...
def matrix_size(mat :ListListPt) : PairII := mat.case {
  NilL => Tup(0,0), // should probably give a runtime error
  Cons(lpt:ListPt,lpts:ListListPt) => Tup(len_l(mat), len(lpt))
};

def matrix_read(mat:ListListPt,i:i64,j:i64) : Pt := mat.case {
  NilL => Empty,
  Cons(lpt:ListPt, lpts:ListListPt) => ifz(j,list_read(lpt,i),matrix_read(lpts,j-1))
};

def matrix_write(mat:ListPtPt,i:i64,j:i64,new_pt:Pt) : ListPtPt := mat.case{
  NilL => NilL,
  ConsL(lpt:ListPt,lpts:ListPtPt) => ifz(i,
    ConsL(list_write(lpt,j,new_pt),lpts),
    ConsL(lpt,matrix_write(mat,i-1,j,new_pt)))
};

def matrix_fold(f:FunLLPtPt,start:ListPtPt,pts:ListPt) : ListListPt := pts.case{
  Nil => start,
  Cons(pt:Pt,pts:ListPt) => matrix_fold(f,f.ApLLPP(start,pt),pts)
};

def maze_map(f:FunPtString,maze:ListPtPt) : ListListStr := maze.case{
  NilL => NilSL,
  ConsL(lpt:ListPt,lpts:ListListPt) => ConsSL(map_s(lpt,f),maze_map(f,lpts))
};

def maze_elm2string() : FunPtString := cocase { ApPtS(pt:Pt) => 
  p.case {
    Pt(x:i64,y:i64) => " _"        
    Empty => " *" 
  }
...
def not_empty(cave,ListListPt,i:i64, j:i64) : Bool := matrix_read(cave,i,j).case{
  Empty => False,
  Pt(x:i64,y:i64) => True
}; 

def cave2maze(cave:ListPtPt) : ListListStr := maze_map(maze_elm2string,cave);

def make_maze(n:i64,m:i64) :ListListStr := 
  ife(n%2,0, 
    NilSL, // should give an error 
    ife(m%2,0, NilLSL, // should give an error 
    let init : FunIntIntPt = cocase { ApIIPt(x:i64) => 
      cocase { ApIPt(y:i64) => ife(x%2,0,ife(y%2,0,Pt(x,y),Empty),Empty)} 
    } in 
    let cave : ListListPt = make_matrix(n,m,init) in
    ...
    let possible_holes : ListPt = concat(forii(0,n,0,m,
      cocase { ApIIPt(i:i64) => 
        cocase { ApIPt(j:i64) => 
          ife(i%2,0,ife(j%2,0,Pt(i,j),Empty),
            ife(j%2,1,Pt(i,j),Empty),Emtpy)
        }
      } in 
    cave_to_maze(pierce_randomly(shuffle(possible_holes),cave))
));

def main(n:i64,m:i64) : i64 := make_maze(n,m);
