...
def enum_from_then_to(from: i64, then: i64, t: i64): List[i64] {
    if from <= t { Cons(from, enum_from_then_to(then, (2 * then) - from, t)) } else { Nil }
}
...
def algb2(x: i64, k0j1: i64, k1j1: i64, yss: List[Pair[i64, i64]]): List[Pair[i64, i64]] {
    yss.case[Pair[i64, i64]] {
        ...
        Cons(p, ys) =>
            p.case[i64, i64] {
                Pair(y, k0j) =>
                    let kjcurr: i64 = if x == y { k0j1 + 1 } else { int_max(k1j1, k0j) };
...
def add_zero(ls: List[i64]): List[Pair[i64, i64]] {
    ls.case[i64] {
        Nil => Nil,
        Cons(h, tl) => Cons(Pair(h, 0), add_zero(tl))
    }
}
...
def in_list(x: i64, ls: List[i64]): Bool {
    ls.case[i64] {
        Nil => False,
        Cons(i, is) => if i == x { True } else { in_list(x, is) }
    }
}
        ...
        False =>
            is_singleton(xs).case[i64] {
                ...
                None =>
                    ...
                    new {
                        apply(x) =>
                            let f1: Fun[List[i64], List[i64]] = (
                                algc(m - m2, n - k, xs2, drop(k, ys))
                            );
