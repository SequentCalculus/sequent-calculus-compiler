...
def map_pair(f:Fun[Pair[i64,i64],i64],l:List[Pair[i64,i64]]) : List[i64]{
  l.case[Pair[i64,i64]]{
    Nil => Nil,
    Cons(p,ps) => Cons(f.Ap[Pair[i64,i64],i64](p),map_pair(f,ps))
  }
}

def algb1(xss:List[i64], yss:List[Pair[i64,i64]]) : List[i64]{
  xss.case[i64]{
    Nil => map_pair(new { Ap(x) => snd_ii(x) }, yss),
    Cons(x,xs) => algb1(xs, algb2(x, 0, 0, yss))
  }
}
def algb_listcomp_fun(listcomp_fun_para:List[i64]) : List[Pair[i64,i64]]{
  listcomp_fun_para.case[i64]{
    Nil => Nil,
    Cons(listcomp_fun_ls_h, listcomp_fun_ls_t) =>  Cons(Pair(listcomp_fun_ls_h, 0), algb_listcomp_fun(listcomp_fun_ls_t))
  }
}
...
def rev_loop(l:List[i64],acc:List[i64]) : List[i64]{
  l.case[i64]{
    Nil => acc,
    Cons(i,is) => rev_loop(is,Cons(i,acc))
  }
}

def reverse(l:List[i64]):List[i64]{
  rev_loop(l,Nil)
}
...
=======
        let l2: List[i64] = reverse(algb(reverse(xs2), reverse(ys)));
        let k: i64 = findk(0, 0, -1, zip(l1, l2));
        new { Ap(x) => 
          let f1 : Fun[List[i64],List[i64]] = (algc(m-m2, n-k, xs2, drop(k, ys)));
          let f2 : Fun[List[i64],List[i64]] = algc(m2, k, xs1, take(k, ys));
          f2.Ap[List[i64],List[i64]](f1.Ap[List[i64],List[i64]](x))
>>>>>>> 2ee4f66 (added lcss)
        }
    }
  }
...
def lcss(xs:List[i64], ys:List[i64]) : List[i64] {
  algc(list_len(xs), list_len(ys), xs, ys).Ap[List[i64],List[i64]](Nil) 
}
