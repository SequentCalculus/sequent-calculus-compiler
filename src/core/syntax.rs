...
pub enum BinOp { Prod,Sum,Sub }
pub enum Ctor { Nil, Cons, Tup }
pub enum Dtor { Hd, Tl, Fst, Snd, Ap }
pub enum Clause<T> { Clause(T,Vec<Variable>,Term) }

pub enum Term { 
    Var(Variable),
    Lit(i64),
    Op(Box<Term>,BinOp,Box<Term>),
    IfZ(Box<Term>,Box<Term>,Box<Term>),
    Let(Variable,Box<Term>,Box<Term>),
    Fun(Name,Vec<Box<Term>>,Vec<Covariable>),
    Constructor(Ctor,Vec<Box<Term>>),
    Destructor(Dtor,Vec<Box<Term>>),
    Case(Box<Term>,Vec<Box<Clause<Ctor>>>),
    Cocase(Vec<Box<Clause<Ctor>>>),
    Lam(Variable,Box<Term>),
    App(Box<Term>,Box<Term>),
    Goto(Box<Term>,Covariable),
    Label(Covariable,Box<Term>)
...
impl fmt::Display for Term {
    fn fmt(&self, f: &mut fmt :: Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Term::Fun(nm,args,coargs) => write!(f,"{}({};{})",nm,show_vec(args),coargs.join(", ")),
