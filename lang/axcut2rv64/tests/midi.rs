...
fn test_midi() {
    ...
    let assembler_code = into_rv64_routine("midi", &pretty(code), arg_num);
    ...
    //    .expect("Cannot write to file tests/asm/midi.rv64.asm");
    let mut file =
        File::open("tests/asm/midi.rv64.asm").expect("Cannot open file tests/asm/midi.rv64.asm");
    let mut reference_code = String::new();
    file.read_to_string(&mut reference_code)
        .expect("Cannot read from file tests/asm/midi.rv64.asm");
