...
fn test_arith() {
    ...
    //    .expect("Cannot write to file tests/asm/arith.rv64.asm");
    let mut file =
        File::open("tests/asm/arith.rv64.asm").expect("Cannot open file tests/asm/arith.rv64.asm");
    let mut reference_code = String::new();
    file.read_to_string(&mut reference_code)
        .expect("Cannot read from file tests/asm/arith.rv64.asm");
