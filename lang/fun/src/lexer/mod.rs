...
pub enum Token {
    ...
    #[regex(r"\p{Ll}[\p{Ll}\p{Lu}\p{No}0-9_']*", |lex| lex.slice().to_string())]
    LowerIdent(String),
    #[regex(r"\p{Lu}[\p{Ll}\p{Lu}\p{No}0-9_']*", |lex| lex.slice().to_string())]
    UpperIdent(String),
    ...
    //
    //
    #[regex(r"0|[1-9][0-9]*", |lex| i64::from_str(lex.slice().as_ref()).unwrap())]
    NumLit(i64),
...
mod lexer_tests {
    ...
    fn doc_comment_1() {
        ...
        assert_eq!(
            ...
            Token::DocComment("/// hello".to_string())
        )
    }
