...
use crate::syntax::{
    {program::{Program, ModuleProgram}, Name, Declaration},
    module_declarations::*,
};
...
fn find_given_file<'a>(module_call: &'a str, path: &'a mut PathBuf, is_import: bool) -> Result<PathBuf, LoaderError> {
    ...
    if reg.is_match(module_call) {
        ...
        for dir in split {
            abs_path.push(dir);
            if !abs_path.exists() {
                return Err(LoaderError::FileNotFound {path_to_file: abs_path.to_str().unwrap().to_owned(),})
            }
        }
                ...
                if std_module_path.is_file() {
                    ...
                    if std_module_path.is_dir() {
                        ...
                        if std_module_path.is_file() {
                            ...
                            Err(LoaderError::FileNotFound {path_to_file: std_module_path.to_str().unwrap().to_owned(),})
                        }
                    }
                    else {
                        Err(LoaderError::FileNotFound {path_to_file: std_module_path.to_str().unwrap().to_owned(),})
