...
fn test_non_linear() {
    ...
    let assembler_code = into_x86_64_routine("nonLinear", &pretty(code), arg_num);

    //let mut file = File::create("tests/asm/nonLinear.x86_64.asm")
    //    .expect("Cannot create file tests/asm/nonLinear.x86_64.asm");
    //file.write_all(&mut assembler_code.as_bytes())
    //    .expect("Cannot write to file tests/asm/nonLinear.x86_64.asm");
    let mut file = File::open("tests/asm/nonLinear.x86_64.asm")
        .expect("Cannot open file tests/asm/nonLinear.x86_64.asm");
    let mut reference_code = String::new();
    file.read_to_string(&mut reference_code)
        .expect("Cannot read from file tests/asm/nonLinear.x86_64.asm");
