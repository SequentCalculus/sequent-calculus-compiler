...
fn test_quad() {
    ...
    let assembler_code = into_x86_64_routine("quad", &pretty(code), arg_num);
    ...
    //    .expect("Cannot write to file tests/asm/quad.x86_64.asm");
    let mut file = File::open("tests/asm/quad.x86_64.asm")
        .expect("Cannot open file tests/asm/quad.x86_64.asm");
    let mut reference_code = String::new();
    file.read_to_string(&mut reference_code)
        .expect("Cannot read from file tests/asm/quad.x86_64.asm");
