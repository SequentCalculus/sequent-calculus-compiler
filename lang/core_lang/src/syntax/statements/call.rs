...
use printer::{DocAllocator, Print, tokens::COMMA};
...
impl Print for FsCall {
    fn print<'a>(
        ...
        let args = alloc.intersperse(
            self.args
                .bindings
                .iter()
                .map(|binding| binding.var.print(cfg, alloc)),
            sep,
        );
        self.name.print(cfg, alloc).append(args.parens())
    }
}
