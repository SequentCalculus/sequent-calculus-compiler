...
pub struct Mu1<C: Chi, S = Statement> {
...
pub type FsMu<C: Chi> = Mu1<C, FsStatement>;
...
impl<C: Chi> Typed for Mu1<C> {
    fn get_type(&self) -> Ty {
        self.ty.clone()
    }
}

impl<C: Chi, S: Print> Print for Mu1<C, S> {
...
impl<C: Chi> From<Mu1<C>> for Term<C> {
    fn from(value: Mu1<C>) -> Self {
        Term::Mu1(value)
    }
}
...
impl<C: Chi> TypedFreeVars for Mu1<C> {
...
impl<C: Chi> Uniquify for Mu1<C> {
    fn uniquify(mut self, max_id: &mut ID) -> Mu1<C> {
